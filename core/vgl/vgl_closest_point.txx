...
vgl_point_3d<T> vgl_closest_point(vgl_cubic_spline_3d<T> const& cspl, vgl_point_3d<T> const& p){
  ...
  vcl_vector<vgl_point_3d<T> > const& knots = cspl.const_knots();
  ...
  while(first || ((vcl_fabs(dcm - dcm0)>tolerance)&&iter<max_iter)){
    dcm0 = dcm;
    first = false;
  ...
  if(iter>=max_iter)
    vcl_cout << " Warning: exceeded num interations in closest point cubic_spline_3d\n";
  return cp;
}
