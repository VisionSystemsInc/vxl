...
class vgl_cubic_spline_3d
{
  ...
  vcl_vector<vgl_point_3d<Type> > knots_; // the spline knots
 ...
 public:
  ...
  bool operator!=(vgl_cubic_spline_3d<Type>const& spl) const { return !operator==(spl); }
  ...
  //  see the implementation of the () operator as an example of their use
  ...
  vgl_plane_3d<Type> normal_plane(Type t){
    vgl_point_3d<Type> p = (*this)(t);
    vgl_vector_3d<Type> n = this->tangent(t);
    return vgl_plane_3d<Type>(n,p);
  }
};
// =================  methods and public functions  ===================
template <class Type>
void vgl_cubic_spline_3d<Type>::coefficients(Type vm1, Type v0, Type v1, Type v2,
...
template <class Type>
void vgl_cubic_spline_3d<Type>::knot_indices(Type t, unsigned& im1, unsigned& i0, unsigned& i1, unsigned& i2, Type& u) const{
...
template <class Type>
  ...
  vcl_vector<vgl_point_3d<Type> > knots = spl.knots();
  unsigned n = static_cast<unsigned>(knots.size());
  if(n!= static_cast<unsigned>(knots_.size())) return false;
...
vgl_vector_3d<Type> vgl_cubic_spline_3d<Type>::tangent(Type t) const{
  ...
  if(t>zero && t<nm1){
    ...
    if(tp>nm1)
      tp = nm1-t;
    vgl_point_3d<Type> pm = (*this)(tm), pp = (*this)(tp);
    tang = pp-pm;
  }
    // Case I t==0
  if(t==zero){
...
template <class Type>
vcl_ostream&  operator<<(vcl_ostream& ostr, vgl_cubic_spline_3d<Type> const& spl){
  if(!ostr){
    vcl_cout << "Bad ostream in write vgl_cubic_spline_3d to stream\n";
    return ostr;
  }
  int ic = 0;
  if(spl.closed())
  ...
  for(unsigned i =0; i<static_cast<unsigned>(knots.size()); i++){
    const vgl_point_3d<Type>& p = knots[i];
    ostr << p.x() << ',' << p.y() << ',' << p.z() << '\n';
  }
  return ostr;
}
...
template <class Type>
vcl_istream&  operator>>(vcl_istream& istr, vgl_cubic_spline_3d<Type>& spl){
  if(!istr){
    vcl_cout << "Bad istream in read vgl_cubic_spline_3d from stream\n";
    return istr;
  }
  int ic;
  Type s;
  ...
  Type x, y, z;
  unsigned char c;
  vcl_vector<vgl_point_3d<Type> > knots;
  ...
  spl = vgl_cubic_spline_3d<Type>(knots, s , closed);
  return istr;
};

#endif // vgl_cubic_spline_3d_h_
