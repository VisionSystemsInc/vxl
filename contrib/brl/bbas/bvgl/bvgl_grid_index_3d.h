...
class bvgl_grid_index_3d{
 ...
 bvgl_grid_index_3d(): has_scalars_(false),has_normals_(false),
    nx_(0), ny_(0), nz_(0), delta_x_(Type(1)), delta_y_(Type(1)), delta_z_(Type(1)), npts_(0), thresh_(Type(1)){}
  bvgl_grid_index_3d(unsigned nx, unsigned ny, unsigned nz, vgl_pointset_3d<Type> ptset, Type thresh = Type(1));
  bvgl_grid_index_3d(unsigned nx, unsigned ny, unsigned nz, vgl_pointset_3d<Type> ptset, vcl_vector<Type> scalars, Type thresh = Type(1));
 ...
 protected:
  bool has_scalars_;
  bool has_normals_;
  vbl_array_3d<vcl_vector<vgl_point_3d<Type > > >   p_grid_;
  vbl_array_3d<vcl_vector<vgl_vector_3d<Type > > >  n_grid_;//optional normal grid
  vbl_array_3d<vcl_vector<Type> >  s_grid_;//optional scalar grid
...
bvgl_grid_index_3d<Type>::bvgl_grid_index_3d(unsigned nx, unsigned ny, unsigned nz, vgl_pointset_3d<Type> ptset, Type thresh):
  ...
  p_grid_ = vbl_array_3d<vcl_vector<vgl_point_3d<Type> > >(nx, ny, nz);
  if(has_normals_)
    n_grid_ = vbl_array_3d<vcl_vector<vgl_vector_3d<Type> > >(nx, ny, nz);
...
bvgl_grid_index_3d<Type>::bvgl_grid_index_3d(unsigned nx, unsigned ny, unsigned nz,
                                             vgl_pointset_3d<Type> ptset, vcl_vector<Type> scalars, Type thresh):
  nx_(nx), ny_(ny), nz_(nz), has_normals_(ptset.has_normals()), npts_(0), thresh_(thresh), has_scalars_(true){
  p_grid_ = vbl_array_3d<vcl_vector<vgl_point_3d<Type> > >(nx, ny, nz);
  s_grid_ = vbl_array_3d<vcl_vector<Type> >(nx, ny, nz);
  if(has_normals_)
    n_grid_ = vbl_array_3d<vcl_vector<vgl_vector_3d<Type> > >(nx, ny, nz);
  bbox_ = vgl_bounding_box(ptset);
  xmin_ = bbox_.min_x();
...
bool bvgl_grid_index_3d<Type>::closest_point(vgl_point_3d<Type> const& p, vgl_point_3d<Type>& pc, Type& scalar) const{
  ...
  Type dmin = vcl_numeric_limits<Type>::max();
