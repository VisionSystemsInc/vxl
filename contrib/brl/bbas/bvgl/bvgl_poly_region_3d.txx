// This is brl/bbas/bvgl/bvgl_poly_region_3d.txx
#ifndef bvgl_poly_region_3d_txx_
#define bvgl_poly_region_3d_txx_
...
bvgl_poly_region_3d<Type>::bvgl_poly_region_3d(vnl_vector<Type> const& u_coefs, vnl_vector<Type> const& v_coefs, vgl_vector_3d<Type> const&  normal,
  ...
  if(vcl_fabs(unit_normal_.z()) < 0.0001){
    vcl_cout << "FATAL! - plane parallel to the z axis a singular condition \n";
    return;
  }
...
vgl_point_3d<Type> bvgl_poly_region_3d<Type>::centroid() const{
  ...
  const vcl_vector<vgl_point_2d<Type> >& pts = poly_2d_[0];
  if(pts.size()==0)
    return vgl_point_3d<Type>(cx, cy, cz);
  double n = Type(0);
  for(vcl_vector<vgl_point_2d<Type> >::const_iterator pit = pts.begin();
...
template vcl_ostream& operator<<(vcl_ostream&, bvgl_poly_region_3d<Type > const& p);\
template vcl_istream& operator>>(vcl_istream&, bvgl_poly_region_3d<Type >& p)

#endif // bvgl_poly_region_3d_txx_
