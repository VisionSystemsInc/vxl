...
class boxm2_vecf_orbicularis_oris : public bvgl_gen_cylinder, public boxm2_vecf_geometry_base
{
 public:
  ...
  boxm2_vecf_orbicularis_oris(vcl_string const& geometry_file, double const& cross_section_interval=0.5);
  //: construct from given central axis, cross sections
  boxm2_vecf_orbicularis_oris(vgl_cubic_spline_3d<double> const& axis, vcl_vector<bvgl_cross_section> const& cross_sects, double const& cross_section_interval=0.5):
  ...
  void display_axis_spline_ascii(vcl_ofstream& ostr, unsigned const& r = 0, unsigned const& g = 255, unsigned const& b = 0) const;
  void display_cross_section_pointsets_ascii(vcl_ofstream& ostr, unsigned const& r = 0, unsigned const& g = 255, unsigned const& b = 0) const;
 ...
 private:
  ...
  vcl_map<vcl_string, double> boundary_knots_;
