...
class deformed_residual_function : public vnl_least_squares_function{
 public:
  ...
  virtual void f(vnl_vector<double> const& x, vnl_vector<double>& fx){
    ...
    defpr_.gamma_            = x[4];
    vcl_cout << "Lambda " << defpr_.lambda_<< "  Gamma " << defpr_.gamma_<< " origin " << defpr_.origin_ << '\n';
...
class deformed_cost_function : public vnl_cost_function{
 public:
  ...
  virtual double f(vnl_vector<double> const& x){
    ...
    //vcl_cout << "n valid " << nvalid << '\n';
    vcl_sort(costs.begin(), costs.end(),vcl_greater<int>());
        double nc = 0.0;
...
boxm2_vecf_fit_fat_pocket::boxm2_vecf_fit_fat_pocket(vcl_string const& neutral_face_ptset_path, vcl_string const& skin_ptset_path,
                                                     vcl_string const& fat_pocket_geo_path):
  pocket_(boxm2_vecf_middle_fat_pocket(fat_pocket_geo_path)), is_right_(false){
  double surface_dist_thresh = 1.0;
  vcl_ifstream neu_istr(neutral_face_ptset_path);
  assert(neu_istr);
  vgl_pointset_3d<double> neu_ptset;
  neu_istr >> neu_ptset;
  neutral_face_.set_pointset(neu_ptset);
  neutral_face_.set_thresh(surface_dist_thresh);
  vcl_ifstream skin_istr(skin_ptset_path);
  assert(skin_istr);
...
boxm2_vecf_fit_fat_pocket::boxm2_vecf_fit_fat_pocket(vcl_string const& neutral_face_ptset_path, vcl_string const& deformed_face_ptset_path,
                                                     vcl_string const& skin_ptset_path, vcl_string const& fat_pocket_geo_path):
  pocket_(boxm2_vecf_middle_fat_pocket(fat_pocket_geo_path)), is_right_(false){
  double surface_dist_thresh = 1.0;
  vcl_ifstream def_istr(deformed_face_ptset_path);
  assert(def_istr);
...
double boxm2_vecf_fit_fat_pocket::fit_neutral(vcl_ostream* outstream, bool verbose){
...
#if 0
double boxm2_vecf_fit_fat_pocket::fit_deformed(vcl_ostream* outstream, bool verbose){
...
double boxm2_vecf_fit_fat_pocket::fit_deformed(vcl_ostream* outstream, bool verbose){
  ...
  vcl_cout << "converged result: lambda " << xm_init[0] << "  org(" << xm_init[1]-iorg.x()<< ' ' << xm_init[2]-iorg.y() << ' ' << xm_init[3]-iorg.z() << ")\n";
...
double boxm2_vecf_fit_fat_pocket::fit_deformed(vcl_ostream* outstream, bool verbose){
  ...
  deformed_cost_function dcf(pocket_, deformed_face_.const_ptset(), &skin_layer_);
  vnl_vector<double> xm_init(5);
  double min_gamma = 0.0, min_lambda = 0.0, min_cost=vcl_numeric_limits<double>::max();
        ...
        for(double dy = 0.0; dy<=0.0; dy+=2.0)
          for(double dz = 0.0; dz<=0.0; dz+=10.0){
            ...
            if(cost<min_cost){
             ...
             vcl_cout << "Lambda " << xm_init[0] << "  Gamma " << xm_init[4] <<  " trans(" << dx << ' ' << dy << ' ' << dz << ")  cost = " << cost << '\n';
          }
        }
  vcl_cout << "converged result: lambda " << min_lambda << " gamma " << min_gamma << "  trans(" << min_dx << ' ' << min_dy << ' ' << min_dz << ")\n";
...
bool boxm2_vecf_fit_fat_pocket::plot_middle_fat_pocket_fit(vcl_ostream& ostr) {
