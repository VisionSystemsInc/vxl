...
boxm2_vecf_orbicularis_oris::boxm2_vecf_orbicularis_oris(vcl_string const& geometry_file, double const& cross_section_interval)
{
  vcl_ifstream istr(geometry_file.c_str());
  if (!istr) {
    vcl_cout << " invalid path for orbicularis oris geometry: " << geometry_file << '\n';
    return;
  }
  this->cross_section_interval_ = cross_section_interval;
  vcl_map<vcl_string, vcl_string> oo_path_map;
  vcl_string component, path;
  while (istr >> component >> path)
    oo_path_map[component] = path;
  vcl_map<vcl_string, vcl_string>::iterator mit;
  if (oo_path_map.find("axis_spline") != oo_path_map.end()) {
    vcl_ifstream sstr(oo_path_map["axis_spline"].c_str());
    this->read_axis_spline(sstr);
    sstr.close();
  }
  if (this->axis_.n_knots() == 0){
    vcl_cout << "load axis from " << oo_path_map["axis_spline"] << " failed!\n";
    return;
  }
  if (oo_path_map.find("cross_section_pointset") != oo_path_map.end()) {
    vcl_ifstream cstr(oo_path_map["cross_section_pointset"].c_str());
...
boxm2_vecf_orbicularis_oris boxm2_vecf_orbicularis_oris::tilt() const
{
  ...
  normalize(rot_axis);
  vcl_cout << "st: " << st << "et: " << et << vcl_endl;
  vcl_cout << "start point: " << startp << vcl_endl;
  vcl_cout << "end   point: " << endp << vcl_endl;
  vcl_cout << "rotation axis: " << rot_axis << vcl_endl;

  // deform the axis
  vcl_vector<vgl_point_3d<double> > def_knots;
  ...
  for (double t = 0; t < n_knots; t+= 1.0)
  {
    ...
    double def_y = rot_lp.length() * vcl_cos(tilt_in_rad);
    double def_z = rot_lp.length() * vcl_sin(tilt_in_rad);
  ...
  vcl_vector<vgl_pointset_3d<double> > cs_ptsets;
  vcl_vector<vgl_plane_3d<double> > cs_planes;
  ...
  vcl_vector<bvgl_cross_section> def_cs;
...
boxm2_vecf_orbicularis_oris boxm2_vecf_orbicularis_oris::scale() const
{
  ...
  vgl_vector_3d<double> e2(0.0, 1.0, 0.0);
  vgl_vector_3d<double> e3(0.0, 0.0, 1.0);
  vcl_vector<vgl_point_3d<double> > deformed_knots;
...
boxm2_vecf_orbicularis_oris boxm2_vecf_orbicularis_oris::deform() const
{
  ...
  double s2 = vcl_pow(params_.lambda_, -params_.gamma_);
  double s3 = 1 / (s1*s2);
  // compute the rotation matrix
  ...
  vcl_cout << "deformation along principal axes: " << s1 << ',' << s2 << ',' << s3 << vcl_endl;
  ...
  vcl_vector<vgl_point_3d<double> > ori_pts = axis.knots();
  vcl_vector<vgl_point_3d<double> > deformed_knots;
  ...
  // construct new cross sections
  vcl_vector<bvgl_cross_section> deformed_cs;
  for (unsigned i = 0; i < nc; i++) {
  ...
  vcl_cout << "number of deformed point set: " << ptsets_deform.npts() << vcl_endl;
  return target;
}
...
boxm2_vecf_orbicularis_oris boxm2_vecf_orbicularis_oris::circular_deform() const
{
  ...
  // l3 = cross_product(l1, l2)
  vcl_cout << "deformation along principal axes: " << s1 << ',' << s2 << ',' << s3 << vcl_endl;
  vcl_cout << "deformation origin: " << c_3d << vcl_endl;
  vcl_cout << "ankle point: " << ankle_pt << vcl_endl;
  vgl_vector_3d<double> e1(1.0, 0.0, 0.0);
  vgl_vector_3d<double> e2(0.0, 1.0, 0.0);
  vgl_vector_3d<double> e3(0.0, 0.0, 1.0);
  vcl_vector<vgl_point_3d<double> > deformed_knots;
...
void boxm2_vecf_orbicularis_oris::display_axis_spline_ascii(vcl_ofstream& ostr, unsigned const& r, unsigned const& g, unsigned const& b) const
{
  ...
  for (double t = 0; t < nd; t+=0.2) {
    ...
    ostr << vcl_setprecision(10) << p.x() << ',' << vcl_setprecision(10) << p.y() << ',' << vcl_setprecision(10) << p.z() << ','
         << vcl_setprecision(10) << n.x() << ',' << vcl_setprecision(10) << n.y() << ',' << vcl_setprecision(10) << n.z() << ','
         << r << ',' << g << ',' << b << '\n';
  }
  ostr.close();
  return;
...
void boxm2_vecf_orbicularis_oris::display_cross_section_pointsets_ascii(vcl_ofstream& ostr, unsigned const& r, unsigned const& g, unsigned const& b) const
{
