...
void test_composite_face()
{
  ...
  vcl_string pc_path = pc_dir + "cristina/cristina_trans_face.txt";
  ...
  if(!vul_file::exists(pc_path)){
      vcl_cout<<"pc path  " << pc_path << " does not exist"<<vcl_endl;
      return;
  } 
    
  if(!vul_file::exists(params_path)){
    vcl_cout<<"pc params file " << params_path << " does not exist"<<vcl_endl;
    return;
  } 
