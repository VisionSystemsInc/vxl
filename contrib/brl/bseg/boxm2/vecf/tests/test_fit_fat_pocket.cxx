...
vgl_point_3d<double> pn7(30.946899414063,-61.403499603271,60.0);
...
#if 0 //extract skin
  vcl_string skin_path = base_path + "skull/skin_orig_no-appearance_sampled-r10-s1.05-t10.txt";
  vgl_pointset_3d<double> skin_pc, coupled_skin_pc;
  vcl_ifstream istr(skin_path.c_str());
  if(!istr)
   return;
  istr >> skin_pc;
  istr.close();
  ...
  vcl_string coupled_skin_pc_path = base_path + "fat_pocket/coupled_skin_pts.txt";
  vcl_ofstream ostr(coupled_skin_pc_path.c_str());
  if(!ostr) return;
  ostr << coupled_skin_pc;
  ostr.close();
#else // extract neutral
  vgl_pointset_3d<double> neutral_pc, coupled_neutral_pc;
  //vcl_string neu_pc_path = base_path + "bs000_N_N_1/bs000_N_N_1_inv_trans.txt";
  vcl_string neu_pc_path = base_path + "bs000_E_HAPPY_0/bs000_E_HAPPY_0_inv_trans.txt";
  vcl_ifstream istr(neu_pc_path.c_str());
  if(!istr)
   return;
  istr >> neutral_pc;
  istr.close();
  ...
  //vcl_string coupled_neutral_pc_path = base_path + "bs000_N_N_1/bs000_N_N_1_coupled_inv_pc.txt";
  vcl_string coupled_neutral_pc_path = base_path + "bs000_E_HAPPY_0/bs000_E_HAPPY_0_coupled_inv_pc.txt";
  vcl_ofstream ostr(coupled_neutral_pc_path.c_str());
  if(!ostr) return;
...
#else   
  vcl_string base_path = "d:/VisionSystems/Janus/RelevantPapers/FacialMusclesExpression/";
  vcl_string neutral_face_pc_path = base_path + "bs000_N_N_1/bs000_N_N_1_coupled_inv_pc.txt";
  vcl_string deformed_face_pc_path = base_path + "bs000_E_HAPPY_0/bs000_E_HAPPY_0_coupled_inv_pc.txt";
  vcl_string skin_pc_path = base_path + "fat_pocket/coupled_skin_pts.txt";
  vcl_string scene_dir = "d:/VisionSystems/Janus/experiments/vector_flow/fat_pocket/";
  vcl_string fat_pocket_geo_path = scene_dir + "middle_fat_pocket_geo.txt";
#if 1
  ...
  double err = ffp.fit_neutral(&vcl_cout, true);
  vcl_cout << "Returned error value " << err << '\n';
  vcl_string fit_error_path = base_path + "bs000_N_N_1/bs000_N_N_1_fit_error_plot.txt";
  vcl_ofstream err_ostr(fit_error_path.c_str());
  if(!err_ostr) return;
  ...
  boxm2_vecf_middle_fat_pocket_params params;
  double err = ffp.fit_deformed(&vcl_cout, true);
  vcl_cout << "Returned error value " << err << '\n';
  vcl_string fit_error_path = base_path + "bs000_E_HAPPY_0/bs000_E_HAPPY_0_fit_error_plot.txt";
  vcl_ofstream err_ostr(fit_error_path.c_str());
  if(!err_ostr) return;
  ffp.plot_middle_fat_pocket_fit(err_ostr);
  err_ostr.close();
