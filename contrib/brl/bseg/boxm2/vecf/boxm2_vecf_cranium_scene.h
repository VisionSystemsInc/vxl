...
class boxm2_vecf_cranium_scene : public boxm2_vecf_articulated_scene
{
 ...
 boxm2_vecf_cranium_scene(): cranium_data_(0),boxm2_vecf_articulated_scene(){}
  ...
  boxm2_vecf_cranium_scene(std::string const& scene_file);
 ...
 std::vector<cell_info> box_cell_centers_;       // cell centers in the target block
  ...
  std::vector<unsigned> cranium_cell_data_index_;           // corresponding data indices
  //      cell_index          cell_index
  std::map<unsigned, std::vector<unsigned> > cell_neighbor_cell_index_; // neighbors of each cranium voxel
